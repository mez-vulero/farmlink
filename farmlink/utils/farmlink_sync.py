            ...
            for created_record in table_changes.get('created', []):
                try:
                    ...
                    if created_record.get('frappe_id'):
                        frappe.logger().info(f" Skipping record with existing frappe_id: {created_record.get('frappe_id')}")
                        continue
                    ...
                    # Handle child tables for primary or secondary processing
                    if watermelon_table in ["primary_processing", "secondary_processing"]:
                        child_field = "processing_output" if watermelon_table == "primary_processing" else "processed_output"
                        if created_record.get(child_field):
                            try:
                                outputs = json.loads(created_record[child_field]) if isinstance(created_record[child_field], str) else created_record[child_field]
                                frappe_data[child_field] = []
                                for item in outputs:
                                    frappe_data[child_field].append({
                                        "grade": item.get("grade"),
                                        "weightkg": item.get("weight"),
                                        "moisture": item.get("moisture")
                                    })
                            except Exception as e:
                                frappe.logger().error(f"Failed to parse {child_field}: {str(e)}")
            ...
            for updated_record in table_changes.get('updated', []):
                try:
                    ...
                    if not frappe.db.exists(frappe_doctype, record_id):
                        ...
                        try:
                            # Convert WatermelonDB data to Frappe format
                            frappe_data = dict(updated_record)
                            ...
                            if watermelon_table in ["primary_processing", "secondary_processing"]:
                                child_field = "processing_output" if watermelon_table == "primary_processing" else "processed_output"
                                if updated_record.get(child_field):
                                    try:
                                        outputs = json.loads(updated_record[child_field]) if isinstance(updated_record[child_field], str) else updated_record[child_field]
                                        frappe_data[child_field] = []
                                        for item in outputs:
                                            frappe_data[child_field].append({
                                                "grade": item.get("grade"),
                    ...
                    # Handle updates to child tables for primary or secondary processing
                    if watermelon_table in ["primary_processing", "secondary_processing"]:
                        child_field = "processing_output" if watermelon_table == "primary_processing" else "processed_output"
                        if updated_record.get(child_field):
                            try:
                                ...
                                frappe.logger().error(f"Failed to update {child_field}: {str(e)}")
            ...
            for deleted_record in table_changes.get('deleted', []):
                try:
                    ...
                    if frappe.db.exists(frappe_doctype, record_id):
                        ...
                        frappe.db.commit()
                        frappe.logger().info(f" Deleted {frappe_doctype}: {record_id}")
